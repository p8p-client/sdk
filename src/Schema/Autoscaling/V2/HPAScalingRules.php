...
#[K8sSchemaRef(name: 'io.k8s.api.autoscaling.v2.HPAScalingRules')]
class HPAScalingRules
{
    /**
     * @param array<int, HPAScalingPolicy>|null $policies                   policies is a list of potential scaling polices which can be used during scaling. If not set, use the default values: - For scale up: allow doubling the number of pods, or an absolute change of 4 pods in a 15s window. - For scale down: allow all pods to be removed in a 15s window.
     ...
     * @param float|string|null                 $tolerance                  tolerance is the tolerance on the ratio between the current and desired metric value under which no updates are made to the desired number of replicas (e.g. 0.01 for 1%). Must be greater than or equal to zero. If not set, the default cluster-wide tolerance is applied (by default 10%).
     *
    ...
    public function __construct(
        ...
        public float|string|null $tolerance = null,
    ) {
    }
}
